...
- [#9](https://github.com/syn54x/skills-plus-plus/pull/9) [`3b83e79`](https://github.com/syn54x/skills-plus-plus/commit/3b83e79b96e5dfdd59bed6929d556a7d844c89be) Thanks [@0x054](https://github.com/0x054)! - Cursor and Codex manifests. `.cursor-plugin/plugin.json` lists the promoted skills explicitly, because Cursor does not discover `skills/<bucket>/<name>/` from a single `./skills/` path (verified with `cursor-agent --plugin-dir`), and ships the `sdd-worker` and `sdd-reviewer` agents plus `hooks/cursor.json` (hooks unverified). `.codex-plugin/plugin.json` and `.agents/plugins/marketplace.json` are included but unverified. `sdd-worker`'s description is no longer invalid YAML, which made Cursor drop the agent; `scripts/check-invocation.py` now rejects unquoted `: ` or ` #` in frontmatter values. `scripts/sync-plugin-version.mjs` keeps every manifest's version in step with `package.json`.

- [#1](https://github.com/syn54x/skills-plus-plus/pull/1) [`62e4e6b`](https://github.com/syn54x/skills-plus-plus/commit/62e4e6b45726897981ecdb2985bd7f8de8351fbc) Thanks [@0x054](https://github.com/0x054)! - Fork identity for `syn54x/skills-plus-plus`. The plugin is now `skills-plus-plus` in the `syn54x` marketplace (`/plugin marketplace add syn54x/skills-plus-plus`, then `/plugin install skills-plus-plus@syn54x`). `setup-matt-pocock-skills` is renamed `setup-syn54x-skills` and `ask-matt` is renamed `ask-syn54x`; every reference, docs page, and link follows. New: ADR 0003 (why the fork extends upstream in place) and `.agents/upstream-sync.md` (how to merge from upstream).

- [#7](https://github.com/syn54x/skills-plus-plus/pull/7) [`8b722d7`](https://github.com/syn54x/skills-plus-plus/commit/8b722d7c19469c1676e7ccf69bf670251c0549f9) Thanks [@0x054](https://github.com/0x054)! - Five general-purpose skills move in from `syn54x/skills`: `scaffold-python-project`, `scaffold-frontend-project` and `prepare-release-notes` in `engineering/`, and `adhd` and `eli5` in `productivity/`. The scaffolds' opt-in flag is now `syn54x-skills`, and their last step hands off to `/setup-syn54x-skills` for the user to run instead of reading that skill's `SKILL.md` and following it by hand, which broke the invocation rules.

- [#2](https://github.com/syn54x/skills-plus-plus/pull/2) [`4c0c473`](https://github.com/syn54x/skills-plus-plus/commit/4c0c473aeeda24f59bc6ddc8317ad82587fe5edb) Thanks [@0x054](https://github.com/0x054)! - Plugin infrastructure for the SDD pipeline. The plugin now ships `hooks/`: a verify gate that stops a worker on an `sdd/*` branch from finishing without a recorded Verify run on a clean tree, and a worktree guard that refuses to remove a worktree with uncommitted or unpushed work. Two repo checks run in CI and as prek hooks: `scripts/check-plugin-skills.sh` (the promoted set matches `plugin.json`, `docs/` and `README.md`) and `scripts/check-invocation.py` (a skill's invocation flags agree, and no skill or agent calls a user-invoked skill).

- [#6](https://github.com/syn54x/skills-plus-plus/pull/6) [`a436f38`](https://github.com/syn54x/skills-plus-plus/commit/a436f388bd210e4d93ece4dfef4cbef2186d7384) Thanks [@0x054](https://github.com/0x054)! - The SDD build skills move in from `syn54x/skills`: `build-epic` (user-invoked orchestrator), and the model-invoked `implement-issue`, `review-pr`, `review-panel`, `close-epic` and `sync-progress`, plus the `sdd-worker` and `sdd-reviewer` agents. Every skill another skill or agent has to start is now model-invoked, which fixes workers stalling on "cannot invoke `/implement-issue`" (ADR 0004). `build-epic` now runs `close-epic` after the PR to `main` merges instead of before. `implement` hands an SDD sub-issue to `implement-issue`. Runtime scripts live inside the skill that uses them, and `progress-comment.sh` no longer passes an unsupported `--arg` to `gh api --jq`.

- [#3](https://github.com/syn54x/skills-plus-plus/pull/3) [`ba7ed4c`](https://github.com/syn54x/skills-plus-plus/commit/ba7ed4c22c927c3b6eae2c7202d8ad21b6de9788) Thanks [@0x054](https://github.com/0x054)! - `setup-syn54x-skills` gains an optional **Section D: SDD pipeline**, offered when the tracker is GitHub. It checks `gh` 2.94+, creates the pipeline's labels plus the five canonical triage labels, offers the `Epic` and `Task` issue types on org repos, writes an `<!-- sdd-routing -->` block below `## Agent skills` (replaced in place on re-run), and can copy the `sdd-implement.yml` and `sdd-review.yml` cloud workflows. This replaces the separate setup skill in `syn54x/skills`, which could only tell the user to run the upstream setup first.

- [#5](https://github.com/syn54x/skills-plus-plus/pull/5) [`de9c27e`](https://github.com/syn54x/skills-plus-plus/commit/de9c27ee5cfd04a6dfb0e91ca48119ac0c67f188) Thanks [@0x054](https://github.com/0x054)! - `to-tickets` gains an SDD pass, read from `to-tickets/SDD.md` when the repo carries the `<!-- sdd-routing -->` block. Before publishing it adds Files owned, Interfaces, Test scenarios and a Verify block to every ticket, runs a no-placeholder gate (failures publish as `needs-info`), and sizes each ticket. It publishes each ticket in the repo its files belong to, with native parent and blocked-by links set at creation, and pins one `<!-- sdd-plan -->` comment on the epic. This replaces `to-tickets-plus` from `syn54x/skills`, which had to call `/to-tickets` and could not, because `to-tickets` is user-invoked.

### Patch Changes

- [#926](https://github.com/mattpocock/skills/pull/926) [`84b5ee5`](https://github.com/syn54x/skills-plus-plus/commit/84b5ee5afd738b6a3484e62509b84b3b573c5be3) Thanks [@mattpocock](https://github.com/mattpocock)! - Add the `implement-spec` skill (in-progress bucket, user-invoked). It takes a spec and its tickets and drives them to a single PR: the tickets are read as a task graph with blocking edges, so implementer subagents run in background worktrees across the ready frontier for concurrency, a merger subagent folds each one back into the PR branch, and the flow closes with `/code-review` before the PR is marked ready.

- [#1092](https://github.com/mattpocock/skills/pull/1092) [`d75dcf1`](https://github.com/syn54x/skills-plus-plus/commit/d75dcf1c5bccfab736ab79f494fa248980546d45) Thanks [@mattpocock](https://github.com/mattpocock)! - Add the `pr` skill (in-progress bucket, model-invoked). It's a reference for the shape a pull request body should take, not a workflow: the template comes first, then a short section per part of it. The summary comes from the primary source (the issue or spec), never inferred from the diff; the body states size and a one-way/two-way door call up front; "the shape of the change" reproduces `show-me` almost verbatim (credited in the skill's `CREDITS.md`), aimed at a diff instead of a conversation; evidence is a before/after pair (visual first, a failing-then-passing test run where no visual exists); and what was deliberately left out gets its own section. Relates to [#521](https://github.com/syn54x/skills-plus-plus/issues/521), [#938](https://github.com/syn54x/skills-plus-plus/issues/938), [#509](https://github.com/syn54x/skills-plus-plus/issues/509), and [#915](https://github.com/syn54x/skills-plus-plus/issues/915).

- [#848](https://github.com/mattpocock/skills/pull/848) [`f02e2ed`](https://github.com/syn54x/skills-plus-plus/commit/f02e2ed3624d031272f8547742d23bf6bca8b072) Thanks [@mattpocock](https://github.com/mattpocock)! - domain-modeling: trigger on discussing codebase terminology and on writing or editing a CONTEXT.md or an ADR directly, replacing the narrower "pin down domain terminology or a ubiquitous language" / "record an architectural decision" phrasing. Also drops the "another skill needs to maintain the domain model" caveat, since that's the invoking skill's job to state explicitly, not this description's.

- [#911](https://github.com/mattpocock/skills/pull/911) [`4f28947`](https://github.com/syn54x/skills-plus-plus/commit/4f289474bad013fe2be8f8769d733f59d9103d6b) Thanks [@mattpocock](https://github.com/mattpocock)! - Quote the `description` front matter in `to-spec`, `code-review`, `setup-matt-pocock-skills`, `writing-fragments`, `writing-shape`, and `wait-what`. An unquoted colon-space left over from the em-dash sweep in [#905](https://github.com/syn54x/skills-plus-plus/issues/905) made each block invalid YAML, so `skills.sh` skipped all six during discovery and they couldn't be listed or installed via `npx skills`.

- [#917](https://github.com/mattpocock/skills/pull/917) [`85f83d3`](https://github.com/syn54x/skills-plus-plus/commit/85f83d3fde1d3a90d5c9a657f6998c79a6c37308) Thanks [@mattpocock](https://github.com/mattpocock)! - grilling: update the round template so consecutive questions are separated by a horizontal rule (`---`) instead of running together.

- [#879](https://github.com/mattpocock/skills/pull/879) [`d419977`](https://github.com/syn54x/skills-plus-plus/commit/d419977fe07d9e1607d3523f3579310bbb076b93) Thanks [@mattpocock](https://github.com/mattpocock)! - grilling: remove em-dashes from `SKILL.md`, replacing them with colons and semicolons so the instructions read as plain text.

- [#905](https://github.com/mattpocock/skills/pull/905) [`e6e9577`](https://github.com/syn54x/skills-plus-plus/commit/e6e957797d8cceb5b351c0dc840369523f9fb8fb) Thanks [@mattpocock](https://github.com/mattpocock)! - Remove every em-dash from the repo's prose (docs, `SKILL.md` files, ADRs, `README.md`, scripts, JSON/YAML metadata), hand-rewriting each sentence with a comma, colon, period, parentheses, or conjunction rather than mechanically substituting the character. `CLAUDE.md`/`AGENTS.md` now says not to reintroduce them.

- [#1](https://github.com/syn54x/skills-plus-plus/pull/1) [`62e4e6b`](https://github.com/syn54x/skills-plus-plus/commit/62e4e6b45726897981ecdb2985bd7f8de8351fbc) Thanks [@0x054](https://github.com/0x054)! - Remove `scaffold-exercises` from `misc/`. It only served upstream's course tooling (`ai-hero-cli`), which this fork doesn't use.

- [#1083](https://github.com/mattpocock/skills/pull/1083) [`6942bff`](https://github.com/syn54x/skills-plus-plus/commit/6942bff8a53666e83a2c621696d2a8e3022429e6) Thanks [@mattpocock](https://github.com/mattpocock)! - retro: classify coding-standards findings as mechanical or judgement calls before writing them. A mechanical violation (a fixed syntactic pattern, a banned API, an import shape, a file-location rule) now gets a deterministic check instead (a linter rule, a pre-commit hook, or a CI job), reserving `CODING_STANDARDS.md` for genuine judgement calls. Automated checks also now flags a repo with no guardrail at all (no pre-commit hook, no CI lint/typecheck/test job) as a finding in its own right.

- [#8](https://github.com/syn54x/skills-plus-plus/pull/8) [`f9a7690`](https://github.com/syn54x/skills-plus-plus/commit/f9a7690daa60a166cb6062d1e83a7a689fbfa927) Thanks [@0x054](https://github.com/0x054)! - `ask-syn54x` routes the SDD flow (epic, hardened sub-issues, `build-epic` or the cloud workflow by size, you merge, `close-epic`) and the new standalones (`adhd`, `eli5`, `prepare-release-notes`, the scaffolds), and drops a stale line saying `diagnosing-bugs` hands off to `improve-codebase-architecture`. `README.md` gains the SDD flow, the size ladder, and the rationale and credits carried over from `syn54x/skills`; `CONTEXT.md` gains the SDD and scaffold glossary.

- [#878](https://github.com/mattpocock/skills/pull/878) [`e3e547b`](https://github.com/syn54x/skills-plus-plus/commit/e3e547b57d549110a0aa6ff40fd7b871c01c76c9) Thanks [@mattpocock](https://github.com/mattpocock)! - Standardize cross-skill invocation on an explicit "call the Skill tool" instruction instead of bare `/skill`-style prose, across `code-review`, `diagnosing-bugs`, `grill-with-docs`, `grill-me`, `improve-codebase-architecture`, `tdd`, `to-spec`, `to-tickets`, `triage`, and `wayfinder`.
...
- [#880](https://github.com/mattpocock/skills/pull/880) [`1dab982`](https://github.com/syn54x/skills-plus-plus/commit/1dab98299c3b81f560026c01b7ebf55ed5d91373) Thanks [@mattpocock](https://github.com/mattpocock)! - Stop skills from trying to reach user-invoked skills through the Skill tool: fix cross-skill references that violated the "no other skill can call it" invariant in `.agents/invocation.md`, in `to-spec`, `wayfinder`, `to-tickets`, `triage`, `code-review`, and `diagnosing-bugs`.

  - `to-spec`, `wayfinder`, `to-tickets`, `triage`, and `code-review` each carried a precondition ("...run `/setup-matt-pocock-skills` if not") that PR [#878](https://github.com/syn54x/skills-plus-plus/issues/878) rewrote into a literal `Call the Skill tool with "setup-matt-pocock-skills"` instruction. `setup-matt-pocock-skills` is user-invoked, so none of these skills (user-invoked or model-invoked) can call it. Reworded all five as instructions for the agent to tell the human to run it instead.
  ...
  - Added a carve-out paragraph to `.agents/invocation.md`'s "Dependencies between them" section: the `Call the Skill tool with "name"` convention only applies when the named skill is model-invoked. This is the section PR [#878](https://github.com/syn54x/skills-plus-plus/issues/878) introduced without reconciling it against the user-invoked/model-invoked invariant stated eight lines above it; the gap is most of why this bug reached six call sites instead of one.

  Fixes [#453](https://github.com/syn54x/skills-plus-plus/issues/453).

- [#904](https://github.com/mattpocock/skills/pull/904) [`594f0f8`](https://github.com/syn54x/skills-plus-plus/commit/594f0f83188921a60d45d63d6cdac509de20df2c) Thanks [@mattpocock](https://github.com/mattpocock)! - wait-what: follow `CONTEXT-MAP.md` to the right `CONTEXT.md` when a repo indexes multiple contexts that way instead of keeping a single root `CONTEXT.md`.
