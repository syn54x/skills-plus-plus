...
Picking a candidate starts a [grilling](../productivity/grilling.md) session over it: constraints, what sits behind the seam, which tests survive, what the deepened interface should look like. The output of that session is a decision, not a diff. From there the normal flow applies: take the decision into [to-spec](../engineering/to-spec.md), then [to-tickets](../engineering/to-tickets.md), then [implement](../engineering/implement.md).
