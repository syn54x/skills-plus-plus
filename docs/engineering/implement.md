...
`implement` builds work that has already been decided. You point it at a ticket, a spec, or the plan you just agreed in the conversation, and it writes the code, drives [tdd](../engineering/tdd.md) at the seams, typechecks as it goes, runs [code-review](../engineering/code-review.md) at the end, and commits to the current branch.
...
You invoke this by typing `/implement` yourself: the agent won't reach for it on its own. It ships with `disable-model-invocation: true`, so no other skill can call it either. Wherever [ask-syn54x](../engineering/ask-syn54x.md) or [to-tickets](../engineering/to-tickets.md) says "then `/implement` per ticket", that is an instruction to you, not something the agent will do unprompted.
...
| Not written down anywhere yet | [grill-with-docs](../engineering/grill-with-docs.md), or [grill-me](../productivity/grill-me.md) if there's no codebase |
| One concrete behaviour you want test-first, with no spec | [tdd](../engineering/tdd.md) directly |
| Already built, and you want it checked | [code-review](../engineering/code-review.md) directly |
...
If the tickets came from [to-tickets](../engineering/to-tickets.md), the tracker they live on was configured by [setup-syn54x-skills](../engineering/setup-syn54x-skills.md). `code-review` reads the same configuration to find the originating spec at close-out.
...
5. Run [code-review](../engineering/code-review.md), then commit to the current branch.

One run covers one ticket. The tickets [to-tickets](../engineering/to-tickets.md) produces are tracer-bullet vertical slices sized to fit a single fresh context window, so the intended rhythm is: clear context, implement one ticket, commit, clear again. Each ticket is self-contained, which is what makes the previous ticket's context disposable.
...
Its neighbours are [to-tickets](../engineering/to-tickets.md), which produces the tickets it consumes and declares the blocking edges that decide their order; [tdd](../engineering/tdd.md), which it drives internally at each seam; and [code-review](../engineering/code-review.md), which it runs before committing. It sits downstream of the planning skills and trusts them. It does not re-validate the shape of what it was handed, so a badly-structured map or a horizontally-layered ticket gets built as written.

That trust is why [wayfinder](../engineering/wayfinder.md) merges onto the chain at [to-spec](../engineering/to-spec.md) rather than looping its map straight into `implement`. Go straight to `implement` from a map only when the effort turned out genuinely small.

[ask-syn54x](../engineering/ask-syn54x.md) is the router over the whole set when you are not sure which flow you are in.
